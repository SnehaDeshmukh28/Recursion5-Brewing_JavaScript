    ...
    # User input for the location name
    location_name = st.text_input("Enter the location name:")

    # Generate button to trigger text generation
    if st.button("Generate Text"):
        # Start a conversation with Gemini and send the location name as a prompt
        convo = model.start_chat(history=[])
        convo.send_message(f"Make a plan for {location_name}")
